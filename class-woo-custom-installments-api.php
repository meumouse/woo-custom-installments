...
if( !class_exists( 'Woo_Custom_Installments_Api' ) ) {
	class Woo_Custom_Installments_Api extends Woo_Custom_Installments_Init {
        ...
        public $license_is_valid;
        private static $_onDeleteLicense = [];

		public function __construct( $plugin_base_file = '') {
            parent::__construct();

            if( isset( $_POST[ 'active_license' ] ) || empty( get_option('woo_custom_installments_license_key') ) ) {
                add_option( 'woo_custom_installments_server_host_option', 'https://meumouse.com/wp-json/license/' );
            }

            $this->server_host = get_option( 'woo_custom_installments_server_host_option' );

            if ( isset( $_POST[ 'active_license' ] ) ) {
                add_option( 'woo_custom_installments_server_host_option', 'https://meumouse.com/wp-json/license/' );
                wp_schedule_single_event( time() + 5, 'woo_custom_installments_delete_server_host_option' );
            }
            
            add_action( 'woo_custom_installments_delete_server_host_option', function() {
                delete_option( 'woo_custom_installments_server_host_option' );
            } );
					...
					if( $this->isTheme) {
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=woo-custom-installments_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		...
		private function _request( $relative_url, $data, &$error = '' ) {
            ...
            // check if server_host in the cache
            $server_host = get_transient( 'woo_custom_installments_server_host' );

            if ( ! $server_host ) {
                $server_host = $this->server_host = "https://meumouse.com/wp-json/license/";
                set_transient( 'woo_custom_installments_server_host', $server_host, 30 * DAY_IN_SECONDS );
            }

            // delete transient cache when deactive license
            if( isset( $_POST[ 'deactive_license' ] ) ) {
                delete_transient( 'woo_custom_installments_server_host' );
            }
                    ...
                    $serverResponse = wp_remote_post($url, $rq_params);
                     ...
                     if (is_wp_error($serverResponse)) {
                        ...
                        if ( ! empty( $serverResponse['body'] ) && ( is_array( $serverResponse ) && 200 === (int) wp_remote_retrieve_response_code( $serverResponse ) ) && $serverResponse['body'] != "GET404" ) {
                            ...
                            set_transient( $transient_name, $cached_response, DAY_IN_SECONDS );
                            return $this->processs_response( $cached_response );
                        }
                    }
                } else {
