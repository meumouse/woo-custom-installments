...
class Frontend {
	...
	public function __construct() {
		if ( Admin_Options::get_setting('enable_installments_all_products') === 'yes' ) {
			...
			add_filter( 'woocommerce_get_price_html', array( $this, 'woo_custom_installments_group' ), 30, 2 );
			...
			if ( Admin_Options::get_setting('hook_payment_form_single_product') === 'before_cart' ) {
				...
				add_action( Admin_Options::get_setting('set_custom_hook_payment_form'), array( $this, 'full_installment' ), 10 );
			} else {
	...
	public function set_values( $return, $price = false, $product = false, $echo = true ) {
		...
		$installments_limit = Admin_Options::get_setting('max_qtd_installments');
		...
		for ( $i = 1; $i <= $installments_limit; $i++ ) {
			...
			if ( Admin_Options::get_setting('set_fee_per_installment') === 'yes' ) {
				...
				$interest_rate = Admin_Options::get_setting('fee_installments_global');
			}
			...
			$max_installments_without_fee = Admin_Options::get_setting('max_qtd_installments_without_fee');
		...
		$min_installment_value = Admin_Options::get_setting('min_value_installments');
	...
	public function woo_custom_installments_debit_card_flags() {
		...
		if ( Admin_Options::get_setting('enable_debit_card_method_payment_form') === 'yes') {
			...
			$debit_card_flag .= '<h4 class="debit-card-method-title">' . Admin_Options::get_setting('text_debit_card_container') . '</h4>';
	...
	public static function display_discount_on_cart() {
		...
		<tr>
		<th><?php echo apply_filters( 'woo_custom_installments_cart_total_title', sprintf( __( 'Total %s', 'woo-custom-installments' ), Admin_Options::get_setting('text_after_price') ) ); ?></th>
		<td data-title="<?php echo esc_attr( apply_filters( 'woo_custom_installments_cart_total_title', sprintf( __( 'Total %s', 'woo-custom-installments' ), Admin_Options::get_setting('text_after_price') ) ) ); ?>"><?php echo wc_price( $total_cart_value - $total_discount ); ?></td>
		</tr>
		<?php
	}
