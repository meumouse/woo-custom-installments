      ...
      <tr class="display-enable-all-interest-options">
         ...
         <td>
            <div class="form-check form-switch <?php echo ( License::is_valid() ) ? '': 'pro-version-notice'; ?>">
               <input type="checkbox" class="toggle-switch <?php echo ( License::is_valid() ) ? '': 'pro-version'; ?>" id="display_tag_interest_checkout" name="display_tag_interest_checkout" value="yes" <?php checked( self::get_setting( 'display_tag_interest_checkout') == 'yes' && License::is_valid() ); ?> />
            </div>
         </td>
      </tr>
            ...
            <td class="wci-title-method-interest-div">
               <div class="input-group wci-method-interest-selector" id="foreach-payment-<?php echo esc_attr( $gateway->id ); ?>-method-discount" name="form-interest-<?php echo esc_attr( $gateway->id ); ?>-method">
                  ...
                  <select class="form-select get-interest-method-payment-method <?php echo ( License::is_valid() ) ? '': 'pro-version-notice'; ?>" id="woo-custom-installments-payment-discounts-type-<?php echo esc_attr( $gateway->id ); ?>" name="woo_custom_installments_interests[<?php echo esc_attr( $gateway->id ); ?>][type]">
                     <option value="fixed" <?php if( isset( $interest_settings[$gateway->id]['type'] ) && $interest_settings[$gateway->id]['type'] == 'fixed' ) { echo 'selected="selected"'; } ?> ><?php echo esc_html__( 'Valor fixo (R$)', 'woo-custom-installments' ) ?></span></option>
                     <option value="percentage" <?php if( isset( $interest_settings[$gateway->id]['type'] ) && $interest_settings[$gateway->id]['type'] == 'percentage' ) { echo 'selected="selected"'; } ?> ><?php echo esc_html__( 'Percentual (%)', 'woo-custom-installments' ) ?></span></option>
                  </select>
               </div>
            </td>
         </tr>
